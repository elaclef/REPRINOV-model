...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	float genetic_gain_for_TB <- float(all_parameters[2, 26]);
	float genetic_gain_for_TP <- float(all_parameters[2, 27]);
	...
	float fixed_nrj_conso_during_milking<-0.0136;//kwh/l of milk from (Hassoun et al,2013)
	float nrj_conso_per_milking_round<-2.06;///kwh/day for a basis of 48 ewes from (Hassoun et al,2013; personal communication)
	...
	float straw_bought;
	float $_milk;
	float ref_MFC <- 70.0 ; // reference fat content in g/l
	float ref_MPC <- 50.0 ; // reference protein content in g/l
	float flock_MFC;
	float flock_MPC;
	float stock_forage_tot <- 0.0 ; // quantity of forage produced
	float stock_grain <- 0.0 ; // quantity of grain produced
	float stock_straw <- 0.0 ; // quantity of straw produced
	...
	reflex sauvegarde1 when: cycle != 0 and every(#year) {
		...
		to: "Results_ROQ/Results_repro_results_percentage.csv" type: csv rewrite: false header: true;
		save
		[current_date.year , self.scenario, self.name , self.production_season_number , last(self.flock_size_current_production_season), self.total_lambing_count,self.total_lambing_work_time ,self.total_lambing_work_time/self.total_lambing_count ,self.nb_of_insemination , self.number_of_AI_lambings ,  self.nb_of_newborn ,  number_of_youngs_from_AI ,  young_mortality , sales_youngs]
		to: "Results_ROQ/Results_repro_results_numbers.csv" type: csv rewrite: false header: true; 
	...
	reflex sauvegarde2 when: current_date > end_grazing_date {
		loop e over: daily_consumed_grass.keys {
			save [self.scenario, self.name ,  e , self.daily_consumed_grass[e]] to: "Results_ROQ/daily_grazing.csv" rewrite: false type: csv;
		}
		
		loop A over: size_grazing_batch.keys {
			save [self.scenario,self.name,  A , self.size_grazing_batch[A]] to: "Results_ROQ/grazing_batch.csv" rewrite: false type: csv;
		}
	...
	reflex sauvegarde3 when: current_date = first(farmer).end_delivery_date add_hours 12 {
		save [current_date.year ,self.scenario, self.name , self.production_season_number ,length(self.ewe_that_produced), last(self.htmp),last(self.htmp)/length(self.ewe_that_produced),self.total_work_during_milking,self.total_work_during_milking/length(self.ewe_that_produced)] to: "Results_ROQ/Results_htmp_and_milking_work.csv" rewrite: false
		header: true type: csv ;
	...
	reflex maj_indicateurs when: (current_date.month = 11 and current_date.day = 16 and current_date.hour = 0) {
		...
		forage_distributed <- feed_distributed['foin1C_lzdvent'] + feed_distributed['foin2C_lzdvent'] + feed_distributed['foin3C_lzdvent'] + feed_distributed['foin1C_rg_apres_deprimage'] + feed_distributed['foin_PT'] + feed_distributed['enrubannage'] + feed_distributed['ensilage'];
		...
		$_diverses_loads <- $hormonal_treatment + $AI + map_param_eco_loads['cost_other_repro'] + EMP * (map_param_eco_loads['cost_vet'] + map_param_eco_loads['cost_casual_labour'] + map_param_eco_loads['cost_CMV'] + map_param_eco_loads['cost_milk_monitoring']);
		...
		ch4_ent <- (coeff_ch4_ent["adult_ewes"] * adult_batch_size  + coeff_ch4_ent["adult_rams"]* length(ram where (each.age>=1)) + coeff_ch4_ent["youngs"] * young_batch_size)*to_CH4_C ; 
		// * to_CH4 to get the value in kg eq CO2
		...
		ch4_manure_bat<-(manure_volatil_solid["adult_ewes"]* adult_batch_size*awms_bat*fe_manure_bat)/1000 +(manure_volatil_solid["adult_rams"]* length(ram where (each.age>=0))*awms_bat*fe_manure_bat)/1000+(manure_volatil_solid["youngs"]* young_batch_size*awms_bat*fe_manure_bat)/1000;
		ch4_manure_past<-(manure_volatil_solid["adult_ewes"]* adult_batch_size*awms_pat*fe_manure_past)/1000 +(manure_volatil_solid["adult_rams"]* length(ram where (each.age>=0))*awms_pat*fe_manure_past)/1000+(manure_volatil_solid["youngs"]* young_batch_size*awms_pat*fe_manure_past)/1000 ;
		...
		save [current_date.year , self.scenario, self.name ,self.sales_youngs , self.nb_culled_fem ,self.nb_culled_mal] to: "Results_ROQ/prod_other_indicators.csv"
		type: csv rewrite: false header: true;
		save [current_date.year ,self.scenario,self.name ,self.forage_distributed,(self.concentrates_distributed+self.grain_distributed),self.straw_distributed, self.stock_forage_tot , self.stock_grain , self.stock_straw] to:
		"Results_ROQ/vegetal_prod_indicators.csv" type: csv rewrite: false header: true;
		save
		...
		to: "Results_ROQ/economic_indicators.csv" type: csv rewrite: false header: true;
		save
		...
		to: "Results_ROQ/economic_indicators_per_litre.csv" type: csv rewrite: false header: true;
		save
		[current_date.year ,self.scenario, self.name , ch4_ent,ch4_ent_per_l_of_milk,  ch4_manure ,ch4_manure_per_l_of_milk, CH4_emission_tot , CH4_emission_per_l_of_milk, nrj_conso ,CO2_emissions_elec, CO2_emissions_elec_per_l_of_milk,CO2_emissions_feed_input, CO2_emissions_feed_input_per_l_of_milk,water_consumption, water_consumption_per_l_of_milk] to: "Results_ROQ1/environnemental_indicators.csv" type: csv rewrite: false header: true;
	}
	...
	// ---------------------------------------------------------------
	reflex sauvegarde4 when: production_season_number = 9 {
		loop d over: lambing_count_days2.keys {
			save [self.scenario, self.name , d , self.lambing_count_days2[d], self.AI_lambing_count_day[d],self.newborn_count_days2[d],self.lambing_work_time[d]] to: "Results_ROQ/Results_LCW_per_day.csv" rewrite: false type: csv;
		}

		loop p over: htmp_days2.keys {
			save [self.scenario, self.name , p , self.htmp_days2[p],self.milking_work_time[p]] to: "Results_ROQ/Results_HTMP_per_day.csv" rewrite: false type: csv;
		}
	...
	reflex ending_simulation when: production_season_number = 9 {
		do pause;
		end_simulation <- true;
	}
...
species sheep {
	...
	reflex young_mortality_calculation when: newborn and current_date = birth_date add_days suckling_duration {
...
species ewe parent: sheep {
	...
	float BCS;
	float MFC;//Milk fat content
	float MPC;//Milk protein content
			...
			if (flip(0.5)) {
				create ewe number: lit_size with: [age::-1] {
					...
					if (myself.AI_lambs) {
						...
						MFC_init_value <- myself.MFC_init_value + genetic_gain_for_TB;
						MPC_init_value <- myself.MPC_init_value + genetic_gain_for_TP;
	...
	reflex end_of_milking when: number_of_milking_days>0 and lactating and nutrition_state = "lactating" and ((last(milk_prod[lact_num]) < prod_min) or (current_date = my_farmer.end_delivery_date)) {
		...
		/*write name + " -> fin de traite" + current_date + nb_de_jours_de_traite;*/
	}
...
species farmer { 
	...
	float flock_TB_initial;
	float flock_TP_initial;
	...
	reflex update_nb_milking_days_TB_TP when: every(#day) {
		if (current_date between (flock_milking_start, end_delivery_date)) {
			...
			flock_MFC<-flock_TB_initial + 0.122 * nbre_days_since_milking_start;
			flock_MPC<-flock_TP_initial + 0.072 * nbre_days_since_milking_start;
		} else {
			nbre_days_since_milking_start <- 0;
			flock_MFC <- flock_TB_initial;
			flock_MPC <- flock_TP_initial;
		}
	...
	reflex other_dates_and_indicators_update when: current_date = ram_introduction or current_date = sponge {
		...
		write current_date+ "taille du troupeau cette année: " + flock_size color:#red;
		number_of_culled_ewes <- int(flock_size * turnover_rate);
		write current_date+"Nombre de brebis à réformer:" + number_of_culled_ewes;
	...
	reflex choose_ewe_to_inseminate when: AI and ((current_date = sponge) or (current_date = ram_introduction)) {
		if (hormon_shot) {
			write "femelles inseminables:" + (flock_size - length(ewe where each.culling_for_age) - length(ewe where each.culling_for_perf));
			...
			/*Here we choose to select the ewes to be inseminated at random in the flock but can be that selection 
				on criterion (age, genet, success last production season...) */
	...
	reflex inseminates_youngs_with_hormone when: AI_youngs and hormon_shot_youngs and (current_date = ewelamb_AI_date_with_hormone) {
		list<ewe> agnelles_candidates_a_gestation <- ewe where (each.renew and each.to_be_inseminate and each.in_heat); /*This step 
		 * must occur after the 1st AI */
	...
	reflex TB_TP_init_milkprod_monitoring_and_renewupdate when: current_date = flock_milking_start {
		flock_TB_initial <- mean((ewe where not each.newborn) collect (each.MFC_init_value));
		flock_TP_initial <- mean((ewe where not each.newborn) collect (each.MPC_init_value));
		write "Initial milk fat content:" + flock_TB_initial;
		write "Initial milk protein content:" + flock_TP_initial;
	...
	reflex choice_of_renew when: current_date = flock_milking_start add_days 21 {
		...
		if (ewe_autorenewal) {
			ask ((number_of_culled_ewes) first (ewelambs_by_birth_date)) {
				renew <- true;
			}

		} else {
			create ewe number: number_of_culled_ewes with: [age::-1] {
				my_farmer <- myself;
				my_farmer.my_ewes << self;
				...
				birth_date <- current_date minus_months 4;
				BCS <- min(5.0, max(1.0, gauss(2.5, 0.3)));
				newborn <- true;
				renew <- true;
				father_index <- rnd(80, 120);
		...
		if (ram_autorenewal) {
			list<ram> ramlambs_by_birth_date <- ram where (each.newborn) sort_by (each.birth_date);
			ask (nb_renew_ram first (ramlambs_by_birth_date)) {
				renew <- true;
			}
		...
		ask (ram where (each.renew and each.age > 0)) {
			renew <- false;
		}

	}

	reflex young_sales when: every(#day) and current_date between (flock_milking_start add_days 21, flock_milking_start add_months 4) {
	...
	reflex involuntary_culling when: (current_date = ram_introduction add_months 10) or (current_date = sponge add_months 10) {
		...
		write "nb involontary culling:" + length(ewe where (each.culling));
	...
	reflex choice_of_ewes_to_cull_for_performances when: (not hormon_shot and current_date = ram_introduction add_months 10) or (hormon_shot and current_date = sponge add_months 10)
	{
		...
		write "nb culled for perf:" + length((ewe where (each.culling_for_perf)))color:#green;
	}
	...
	reflex calcul_lambing_period_andfertility_rate when: (current_date = ram_introduction add_months 10) or (current_date = sponge add_months 10) {
		flock_size <- length(ewe where each.renew)+length(ewe where (not each.renew and each.age>=0)) - length(ewe where each.culling_for_age) - length(ewe where each.culling_for_perf); //remove the reforms that are not yet gone but will not be reproduced
		write "taille de troupeau à 1 mois de la repro:" + flock_size color: #red;
...
species feed {
	...
	//find the feed name corresponding to the feed type
		...
		DMI <- float(mat_alim[3, (feed_type) - 1]);
		...
		if (feed_category = "concentrates") {
			daily_energy_intake <- UF * feed_qty * DMI;
			daily_PDIN_intake <- PDIN * feed_qty * DMI;
			daily_PDIE_intake <- PDIE * feed_qty * DMI;
		}
	...
	reflex update_feed when: every(#day) {
		if (current_date between (deb_period minus_days 1, end_period add_days 1)) {
			...
			if ((diet_group = 4) and (batch_name = "maintenance")) { 
				...
				if size_batch_9 > 0 {
					if (feed_name != 'herbe') {
						...
						//write current_date +sample(feed_name)+sample (size_batch_3)+"nom de lot:" + batch_name + "taille de lot:" + size_batch[batch_name]+"quantité herbe consommée:"+ daily_consumed_grass[current_date];
			...
			if (batch_name = "suckling") {
				...
				//write current_date + "suckling: " + size_batch_4;
				if size_batch_4 > 0 {
					if (feed_name != 'herbe') {
			...
			if (batch_name = "lactating") {
				...
				//write current_date + "lactating: " + size_batch_5;
				if size_batch_5 > 0 {
					if (feed_name != 'herbe') {
			...
			if (batch_name = "male") {
				...
				//write current_date + "male: " + size_batch_6;
				if size_batch_6 > 0 {
					if (feed_name != 'herbe') {
...
experiment 'Exploration scenarios' type: gui {
	...
	action _init_ {
		save "Scenario,Sim_name,date,ewes_lambing_per_day,AI_lambing_count, lambs_born_per_day,lambing work" to: "Results_ROQ/Results_LCW_per_day.csv" rewrite: false type: csv;
		save "Year,Scenario,Sim_name,production_season,flock_size,fertility_rate,AI%_in_the_flock,%_of_ewes_lambing_from_AI,% of youngs born from AI,%_ewes_renewal_from_AI,%rams_renewal_from_AI" to:
		"Results_ROQ/Results_repro_results_percentage.csv" rewrite: false header: true type: csv ;
		save "Year,Scenario,Sim_name,production_season,flock_size ,tlc,work_time,work_time_per_ewe,Nb ewes inseminated,Nb_of_ewes_lambing_from_AI,Nb_young_born,Nb_young_born_from_AI,Nb_of_young_dead,Nb_agneaux_vendus"
		to: "Results_ROQ/Results_repro_results_numbers.csv" type: csv rewrite: false header: true;
		save "Scenario,Sim_name,date, daily_grazing" to: "Results_ROQ/daily_grazing.csv" rewrite: false type: csv;
		save "Year,Scenario,Sim_name,production_season,ewe_that_produced,htmp,htmp_per_ewe,work_time,work_time_per_ewe" type: csv to: "Results_ROQ/Results_htmp_and_milking_work.csv" rewrite: false header: true;
		save "Scenario,Sim_name, Date, htmp_per_day, work_per_day" to: "Results_ROQ/Results_HTMP_per_day.csv" rewrite: false type: csv;
		string lili <- "Year,Scenario,Sim_name";
			loop v over: ["foin1C_rg_apres_deprimage","orge","complementaire_tourteau","herbe","foin1C_lzdvent","foin_PT","foin2C_lzdvent","premier_age","foin3C_lzdvent","paille","alim_complet_agnelles","ensilage","enrubannage"] {
				lili <- lili + "," +v;
			}
			save lili to:"Results_ROQ/distributed_feed.csv" type: csv rewrite: false header: true ;
		save "Year,Scenario,Sim_name,sales_young,culled_fem,culled_male" to: "Results_ROQ/prod_other_indicators.csv" type: csv rewrite: false header: true;
		save "Year,Scenario,Sim_name,conso_forage,conso_concentrates,conso_straw,stock_forage_tot,stock_grain,stock_straw" to: "Results_ROQ/vegetal_prod_indicators.csv" type: csv rewrite: false header: true;
		save "Year,Scenario,Sim_name,$_gross_product_milkprod_workshop,$_surf_vege_product,$_gross_product_without_subsidies,$_op_loads_milkprod_workshop,$_surf_vege_loads,$_operating_loads,$gross_marging_whithout_subsidies"
		to: "Results_ROQ/economic_indicators.csv" type: csv rewrite: false header: true;
		save "Year,Scenario,Sim_name,$_gross_product_milkprod_workshop,$_surf_vege_product,$_gross_product_without_subsidies,$_op_loads_milkprod_workshop,$_surf_vege_loads,$_operating_loads,$gross_marging_whithout_subsidies"
		to: "Results_ROQ/economic_indicators_per_litre.csv" type: csv rewrite: false header: true;
		save "Year,Scenario,Sim_name,CH4_ent,CH4_ent/l,CH4_manure,CH4_manure/l,CH4_tot, CH4_tot/l,nrj_conso,CO2_elec,CO2_elec/l,CO2_feed_input,CO2_feed_input/l,water_conso,water conso/l" to: "Results_ROQ/environnemental_indicators.csv" rewrite: false type: csv;
		save "Scenario,Sim_name,date,nb_maintening, nb_gestating,nb_suckling,nb_lactating" to: "Results_ROQ/Results_ewes_physio_stage.csv" rewrite: false type: csv;
		save "Scenario,Sim_name,date, nb_of_sheep_grazing" to: "Results_ROQ/grazing_batch.csv" rewrite: false type: csv;
		
		
		loop i from: 0 to: 2 {
			loop times: 75 {
				create simulation with: [scenario::"scenar" + i,seed::rnd(1.0)];
			}
